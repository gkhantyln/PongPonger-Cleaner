# === ADVANCED PONGPONGER CLEANER WITH MENU OPTIONS ===
...
Write-Host "    ADVANCED PONGPONGER CLEANER TOOL     " -ForegroundColor Cyan
Write-Host "=========================================" -ForegroundColor Cyan
Write-Host ""
...
function Show-MainMenu {
    ...
    Write-Host "1. Automatic Full Scan and Cleanup" -ForegroundColor Green
...
function Write-Log {
    ...
    # Write to log file
    $logMessage | Out-File -FilePath "$env:TEMP\PongPongerCleaner.log" -Append
}
...
function Clean-HostsFile {
    ...
    if (Test-Path $hostsPath) {
        try {
            ...
            Copy-Item -Path $hostsPath -Destination "$hostsPath.backup" -Force -ErrorAction SilentlyContinue
            
            # Write cleaned content
            $newContent | Out-File -FilePath $hostsPath -Encoding ASCII -Force
            Write-Host "Hosts file cleaned and backed up to $hostsPath.backup" -ForegroundColor Green
        } catch {
...
function Start-AutomaticCleanup {
    ...
    Write-Host "Note: Please manually check browser extensions:" -ForegroundColor Yellow
...
function Start-ManualProcess {
    do {
        ...
        Write-Host "---------------------------" -ForegroundColor Yellow
        Write-Host "1. Scan and clean registry" -ForegroundColor Green
        Write-Host "2. Scan and clean scheduled tasks" -ForegroundColor Green
        Write-Host "3. Scan and clean files/folders" -ForegroundColor Green
        Write-Host "4. Scan and clean browser shortcuts" -ForegroundColor Green
        Write-Host "5. Scan and clean hosts file" -ForegroundColor Green
        Write-Host "6. Scan and clean services" -ForegroundColor Green
...
do {
    Show-MainMenu
    $userChoice = Read-Host "Select an option (1-9)"
    ...
    switch ($userChoice) {
        ...
        "9" {
            Write-Host ""
            Write-Host "Thank you for using Advanced PongPonger Cleaner!" -ForegroundColor Cyan
            Write-Host "Log file saved to: $env:TEMP\PongPongerCleaner.log" -ForegroundColor White
            Write-Host ""
            exit
        }
        default {
